...
def plot_example_response(axs, odor_times, i, model, predictions, data):
    ...
    def plot_func(X, axs, colors, ylims=(-0.1, 0.75), alpha=1, lw=1):
...
def main(argv):
    ...
    PdfPlotter(filename.format('summary'), fixed_margins=margins)
    plt.figure(figsize=(4.25, 2.5), dpi=300)
    gs1 = matplotlib.gridspec.GridSpec(1, 1, hspace=0.35)
    for j, trial in enumerate(order.values.flatten()):
        gs0 = gs1[j].subgridspec(1, 5)
