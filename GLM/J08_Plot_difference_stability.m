...
if ~isempty(timecell_sigmdl{session,k})    
    ...
    good_stability{session,k} = [good_stability{session,k}; stability{session,k}(i)];
    end
end

if ~isempty(timecell_badmdl{session,k}) 
    if ismember(i,timecell_badmdl{session,k}(:,1))
    bad_stability{session,k} = [bad_stability{session,k}; stability{session,k}(i)];
    end
end
...
figure
% subplot(2,1,1)
histogram(plot_good,-0.2:0.05:0.6,'Normalization','probability')
hold on
histogram(plot_bad,-0.2:0.05:0.6,'Normalization','probability')
legend("good","other")
%%
